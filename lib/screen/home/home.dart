...
class _HomeState extends State<Home> {
  ...
  List<Comprimido> listaTodosComprimidos = <Comprimido>[];
  final Repository _repository = Repository();
  List<Comprimido> comprimidosDiarios = <Comprimido>[];

  //calendario dias
  final CalendarioDiaModel _dias = CalendarioDiaModel(ano:0, mes:0, numeroDia: 0, confirmado: false , diario:' ');
  late List<CalendarioDiaModel> _listaDias;
  ...
  Future setData() async {
    ...
    (await _repository.getAllData("Comprimidos"))!.forEach((comprimidoMap) {
      listaTodosComprimidos.add(Comprimido(id:0, nome: '', idNotificacao:0, diasRecorrentes: 0, tempo:0, formMedicamento: '',tipo:'', quantidade:'' ).comprimidoMapToObject(comprimidoMap));
    });
